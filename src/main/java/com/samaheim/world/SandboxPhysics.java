...
public final class SandboxPhysics {
    ...
    private static final float GRAVITY = 18.5f, JUMP_SPEED = 6.2f, TERMINAL_FALL_SPEED = -32f;
    ...
    public static HorizontalResult resolveHorizontal(float fromX, float fromZ, float toX, float toZ, float playerRadius, List<CircleBlocker> blockers) {
        ...
        List<CircleBlocker> safe = blockers==null?List.of():blockers;
        float radius=Float.isFinite(playerRadius)?Math.max(0f,playerRadius):0f;
        float dx=toX-fromX,dz=toZ-fromZ,d=(float)Math.sqrt(dx*dx+dz*dz);
        int steps=Math.max(1,(int)Math.ceil(d/MAX_HORIZONTAL_STEP)); float sx=dx/steps,sz=dz/steps,x=fromX,z=fromZ; boolean blocked=false;
        for(int i=0;i<steps;i++){
            Depenetration dep=depenetrate(x+sx,z+sz,x,z,radius,safe);
            if(dep.blocked){ blocked=true; Depenetration xo=depenetrate(x+sx,z,x,z,radius,safe), zo=depenetrate(x,z+sz,x,z,radius,safe);
                float fp=progressSq(x,z,dep.x,dep.z),xp=progressSq(x,z,xo.x,xo.z),zp=progressSq(x,z,zo.x,zo.z);
                if(xp>fp&&xp>=zp)dep=xo; else if(zp>fp)dep=zo; }
            x=dep.x; z=dep.z;
        }
        return new HorizontalResult(x,z,blocked);
    }

    private static Depenetration depenetrate(float candidateX,float candidateZ,float fallbackX,float fallbackZ,float playerRadius,List<CircleBlocker> blockers){
        float x=candidateX,z=candidateZ; boolean blocked=false;
        for(int pass=0;pass<MAX_DEPENETRATION_PASSES;pass++){
            CircleBlocker chosen=null; float chosenPenetration=0f;
            for(CircleBlocker b:blockers){
                if(!valid(b))continue; float min=playerRadius+Math.max(0f,b.radius())+COLLISION_SKIN; if(min<=EPS)continue;
                float dx=x-b.x(),dz=z-b.z(),dist=(float)Math.sqrt(Math.max(0f,dx*dx+dz*dz)); float penetration=min-dist;
                if(penetration>EPS && (chosen==null || penetration>chosenPenetration+EPS || (Math.abs(penetration-chosenPenetration)<=EPS && tieBefore(b,chosen)))) { chosen=b; chosenPenetration=penetration; }
            }
            if(chosen==null)break; blocked=true;
            float dx=x-chosen.x(),dz=z-chosen.z(),dist=(float)Math.sqrt(dx*dx+dz*dz);
            if(dist<=EPS){ dx=fallbackX-chosen.x(); dz=fallbackZ-chosen.z(); dist=(float)Math.sqrt(dx*dx+dz*dz); if(dist<=EPS){dx=1f;dz=0f;dist=1f;} }
            float min=playerRadius+Math.max(0f,chosen.radius())+COLLISION_SKIN, push=min/dist;
            x=chosen.x()+dx*push; z=chosen.z()+dz*push;
        }
    ...
    public static VerticalResult stepVertical(float currentEyeY,float velocityY,float groundY,float eyeHeight,boolean jumpRequested,float dt){
        ...
        float frame=Math.max(0f,Math.min(dt,0.05f)),floor=groundY+Math.max(0f,eyeHeight); boolean grounded=currentEyeY<=floor+0.055f&&velocityY<=0.15f; float v=velocityY;
        ...
        if(!grounded)v=Math.max(TERMINAL_FALL_SPEED,v-GRAVITY*frame); float y=currentEyeY+v*frame;
        if(y<=floor){y=floor;v=0f;grounded=true;} return new VerticalResult(y,v,grounded);
    }
