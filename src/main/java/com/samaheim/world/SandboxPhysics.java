...
public final class SandboxPhysics {
    ...
    public static HorizontalResult resolveHorizontal(float fromX,float fromZ,float toX,float toZ,float playerRadius,List<CircleBlocker> blockers){
        ...
        for(int i=0;i<steps;i++){
            ...
            if(dep.blocked){blocked=true;Depenetration xo=depenetrate(x+sx,z,x,z,radius,safe),zo=depenetrate(x,z+sz,x,z,radius,safe);
                float fp=directionalProgress(x,z,dep.x,dep.z,sx,sz),xp=directionalProgress(x,z,xo.x,xo.z,sx,sz),zp=directionalProgress(x,z,zo.x,zo.z,sx,sz);
                if(xp>fp+EPS&&xp>=zp)dep=xo;else if(zp>fp+EPS)dep=zo;
            }
            x=dep.x;z=dep.z;
        }
    ...
    public static VerticalResult stepVertical(float currentEyeY,float velocityY,float groundY,float eyeHeight,boolean jumpRequested,float dt){
        ...
        float frame=Math.max(0f,Math.min(dt,0.05f)),floor=groundY+Math.max(0f,eyeHeight),separation=currentEyeY-floor;
        boolean near=separation<=0.055f||(velocityY<=0.15f&&separation>=0f&&separation<=GROUND_SNAP_DOWN);boolean grounded=near&&velocityY<=0.15f;float v=velocityY;
