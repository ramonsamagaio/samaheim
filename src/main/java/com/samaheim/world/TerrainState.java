...
public final class TerrainState {
    ...
    public TerrainState(long seed, float halfExtent, int minimumCells, float maxDelta) {
        if (!Float.isFinite(halfExtent) || !Float.isFinite(maxDelta) || halfExtent <= 0f || minimumCells < 2 || maxDelta <= 0f) {
    ...
    public int level(float worldX, float worldZ, float radius, float targetHeight, float maxStep) {
        if (!validBrush(worldX, worldZ, radius, maxStep) || !Float.isFinite(targetHeight)) return 0;
        int changed = 0;
        ...
        for (int z = minZ; z <= maxZ; z++) {
            ...
            for (int x = minX; x <= maxX; x++) {
                ...
                if (Math.abs(step) > EPSILON && setLevelOffset(i, levelOffsets[i] + step)) {
                    markDirty(x, z);
                    changed++;
                }
            }
    ...
    /**
     * Smooths only the fine hoe layer. Large cliffs still require coarse raising/lowering,
     * which keeps the system close to Valheim's separation between sculpting and leveling.
     */
    public int smooth(float worldX, float worldZ, float radius, float strength) {
        ...
        for (int i = 0; i < smoothScratch.length; i++) {
            smoothScratch[i] = baseHeights[i] + sculptDeltas[i] + levelOffsets[i];
        }
        ...
        for (int z = minZ; z <= maxZ; z++) {
            ...
            for (int x = minX; x <= maxX; x++) {
                ...
                float neighborAverage = neighborhoodAverage(x, z, smoothScratch);
                int i = index(x, z);
                float sculpted = baseHeights[i] + sculptDeltas[i];
                float targetOffset = clamp(neighborAverage - sculpted, -MAX_LEVEL_OFFSET, MAX_LEVEL_OFFSET);
                float amount = strength * brushWeight(distance, radius);
                float next = moveToward(levelOffsets[i], targetOffset, amount);
                if (setLevelOffset(i, next)) {
                    markDirty(x, z);
                    changed++;
                }
            }
    ...
    /** Returns local terrain inclination using the final edited surface. */
    public float slopeDegrees(float worldX, float worldZ, float sampleRadius) {
        float r = Math.max(cellSize, Math.abs(sampleRadius));
        float dx = sampleHeight(worldX + r, worldZ) - sampleHeight(worldX - r, worldZ);
        float dz = sampleHeight(worldX, worldZ + r) - sampleHeight(worldX, worldZ - r);
        float rise = (float) Math.sqrt(dx * dx + dz * dz);
        float run = 2f * r;
        return (float) Math.toDegrees(Math.atan2(rise, run));
    }

    /** Maximum height spread in a footprint, useful for build placement checks. */
    ...
    public boolean isBuildable(float worldX, float worldZ, float radius, float maxSlopeDegrees, float maxVariation) {
        return Float.isFinite(maxSlopeDegrees) && Float.isFinite(maxVariation)
                && maxSlopeDegrees >= 0f && maxVariation >= 0f
                && slopeDegrees(worldX, worldZ, Math.max(radius * 0.5f, cellSize)) <= maxSlopeDegrees
                && heightVariation(worldX, worldZ, radius) <= maxVariation;
    }

    public int modifiedSampleCount() {
        int count = 0;
        for (int i = 0; i < sculptDeltas.length; i++) {
            if (Math.abs(sculptDeltas[i]) > EPSILON || Math.abs(levelOffsets[i]) > EPSILON) count++;
        }
        return count;
    }

    /**
     * Returns and clears the smallest edited vertex rectangle since the last consume.
     * A renderer can use this to update only the affected mesh patch instead of rebuilding the world.
     */
    public DirtyRegion consumeDirtyRegion() {
        if (dirtyMinX == Integer.MAX_VALUE) return null;
        DirtyRegion region = new DirtyRegion(dirtyMinX, dirtyMinZ, dirtyMaxX, dirtyMaxZ);
        resetDirtyRegion();
        return region;
    }

    public void clearEdits() {
        Arrays.fill(sculptDeltas, 0f);
        Arrays.fill(levelOffsets, 0f);
        markDirty(0, 0);
        markDirty(cells, cells);
    }
    ...
    private int sculptBrush(float worldX, float worldZ, float radius, float amount) {
        if (!validBrush(worldX, worldZ, radius, Math.abs(amount)) || amount == 0f) return 0;
        int changed = 0;
        int minX = gridMin(worldX - radius), maxX = gridMax(worldX + radius);
        int minZ = gridMin(worldZ - radius), maxZ = gridMax(worldZ + radius);
        for (int z = minZ; z <= maxZ; z++) {
    ...
    private float neighborhoodAverage(int x, int z, float[] source) {
        float total = 0f;
        int count = 0;
        for (int dz = -1; dz <= 1; dz++) {
            int nz = clampIndex(z + dz);
            for (int dx = -1; dx <= 1; dx++) {
                int nx = clampIndex(x + dx);
                total += source[index(nx, nz)];
                count++;
            }
        }
        return total / count;
    ...
    private void markDirty(int x, int z) {
        dirtyMinX = Math.min(dirtyMinX, clampIndex(x));
        dirtyMinZ = Math.min(dirtyMinZ, clampIndex(z));
        dirtyMaxX = Math.max(dirtyMaxX, clampIndex(x));
        dirtyMaxZ = Math.max(dirtyMaxZ, clampIndex(z));
    }
    ...
    private boolean validBrush(float worldX, float worldZ, float radius, float strength) {
        return Float.isFinite(worldX) && Float.isFinite(worldZ) && Float.isFinite(radius) && Float.isFinite(strength)
    ...
    public record DirtyRegion(int minX, int minZ, int maxX, int maxZ) {
        public int width() { return maxX - minX + 1; }
        public int height() { return maxZ - minZ + 1; }
        public int sampleCount() { return width() * height(); }
    }
}
