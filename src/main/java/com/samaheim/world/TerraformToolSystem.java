...
public final class TerraformToolSystem {
    ...
    private static final float EDGE_BRUSH_MARGIN = 0.55f;

    private TerraformToolSystem() { }

    public enum Mode {
        LEVEL("Level ground", 0, 4f), RAISE("Raise ground", 2, 7f), LOWER("Dig ground", 0, 6f),
        SMOOTH("Smooth ground", 0, 5f), RESTORE("Restore ground", 0, 3f);
        private final String label; private final int stoneCost; private final float staminaCost;
        Mode(String label, int stoneCost, float staminaCost) { this.label = label; this.stoneCost = stoneCost; this.staminaCost = staminaCost; }
        public String label() { return label; } public int stoneCost() { return stoneCost; } public float staminaCost() { return staminaCost; }
        public Mode next() { Mode[] values = values(); return values[(ordinal() + 1) % values.length]; }
    }
    ...
    public static Result apply(TerrainState terrain, Mode mode, float worldX, float worldZ,
        ...
        if (!Float.isFinite(worldX) || !Float.isFinite(worldZ) || !Float.isFinite(standingHeight)
                || !Float.isFinite(requestedRadius) || requestedRadius <= 0f) return Result.denied("Invalid terrain target.");
        ...
        if (mode == Mode.SMOOTH && roughness <= SMOOTH_NOOP_ROUGHNESS) return Result.denied("The ground is already smooth here.");
        if (mode == Mode.LEVEL && roughness <= LEVEL_NOOP_ROUGHNESS && Math.abs(centerHeight - standingHeight) <= LEVEL_NOOP_HEIGHT_ERROR)
        ...
        if (stoneAvailable < quotedStoneCost) return Result.denied("Raise ground needs " + quotedStoneCost + " stone for this brush size.");
        ...
        String detail = switch (mode) {
            case LEVEL -> " leveled around your footing."; case RAISE -> " packed into a stable mound.";
            case LOWER -> " cut into the earth."; case SMOOTH -> " softened the sharp edges.";
    ...
    public record Result(boolean applied, int changedSamples, int stoneSpent, float staminaSpent, String message) {
        private static Result denied(String message) { return new Result(false, 0, 0, 0f, message); }
    }
}
